...
var MILLISECONDS_IN_MINUTE$1 = 1000 * 60;
...
function formatDistanceStrict(dirtyDate, dirtyBaseDate) {
  ...
  var minutes = milliseconds / MILLISECONDS_IN_MINUTE$1;
  var timezoneOffset = getTimezoneOffsetInMilliseconds(dateRight) - getTimezoneOffsetInMilliseconds(dateLeft); // Use DST-normalized difference in minutes for years, months and days;
  // use regular difference in minutes for hours, minutes and seconds.

  var dstNormalizedMinutes = (milliseconds - timezoneOffset) / MILLISECONDS_IN_MINUTE$1;
  var unit;
  ...
  if (unit === 'second') {
    ...
    var days = roundingMethodFn(dstNormalizedMinutes / MINUTES_IN_DAY$1);
    return locale$1.formatDistance('xDays', days, localizeOptions); // 1 up to 12 months
  } else if (unit === 'month') {
    var months = roundingMethodFn(dstNormalizedMinutes / MINUTES_IN_MONTH$1);
...
function lightFormat(dirtyDate, formatStr) {
  ...
  var timezoneOffset = getTimezoneOffsetInMilliseconds(originalDate);
  var utcDate = subMilliseconds(originalDate, timezoneOffset);
  var tokens = formatStr.match(formattingTokensRegExp$2); // The only case when formattingTokensRegExp doesn't match the string is when it's empty
